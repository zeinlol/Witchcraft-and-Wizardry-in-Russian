...
execute as @s[scores={relogTimer=0}] store result bossbar minecraft:player4health value run scoreboard players operation @s currentHealth = @s tmp
execute as @s[scores={relogTimer=1..}] run bossbar set minecraft:player4health value 200
execute as @s[scores={relogTimer=1..}] run scoreboard players set @s currentHealth 200

bossbar set minecraft:player4health name [{"score":{"name":"@s","objective":"currentHealth"},"bold":true,"color":"white"},{"text":" / ","bold":false,"color":"white"},{"score":{"name":"@s","objective":"maxHealth"},"bold":true,"color":"white"}]