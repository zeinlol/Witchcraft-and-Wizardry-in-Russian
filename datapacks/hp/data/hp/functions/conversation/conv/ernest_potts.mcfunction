...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" При этом... у меня все еще есть ключ от подземелья наверху. И если кто-то придет с чашей, я щедро заплачу им за это. \\n\\n\\n\",\"color\":\"white\",\"bold\":\"false\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=1}] run data merge entity @s {CustomName:"{\"text\":\"±ˈˈ Взять (300 ≈)\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
