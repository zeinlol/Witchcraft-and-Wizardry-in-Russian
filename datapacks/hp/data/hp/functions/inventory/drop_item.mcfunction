...
execute as @s[tag=editAddItems] unless entity @e[tag=editAddItemsTarget,limit=1] run title @s subtitle {"text":"Нужно находиться рядом с контейнером, чтобы изменить его содержимое.","color":"red"}
