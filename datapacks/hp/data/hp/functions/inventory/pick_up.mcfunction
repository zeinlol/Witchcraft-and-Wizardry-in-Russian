...
# Set title
execute if entity @s[tag=!showMaraudersMap,tag=!inQuestBook,tag=!inSettings,tag=!showSpellSelection,tag=!inInventory,tag=!invisCape,tag=!omnioculars,tag=!stunnedByMandrake,tag=!earmuffs,tag=play] run title @s actionbar ["",{"text":"Взято: ","color":"gold"},{"score":{"name":"@s","objective":"pickUpAmount"},"color":"gold"},{"text":" ","color":"gold"},{"selector":"@e[tag=specialHeader,limit=1]","color":"white"}]
