...
execute as @s[scores={timeSinceSpawn=0}] unless score @s spawnCount matches 0 at @s if entity @p[distance=..50] unless score @s spawnCount matches 0 if score @s maxCreatureCount > @s creatureCount at @s run function hp:creatures/pixie_gnome/nest_summon_pixie
...
# Count each pixie gnomes belonging to this nest.
