...
execute as @s[scores={timeSinceSpawn=0}] unless score @s spawnCount matches 0 at @s if entity @p[distance=..50] unless score @s spawnCount matches 0 if score @s maxCreatureCount > @s creatureCount at @s run function hp:creatures/rat/nest_summon_rat
...
# Count each rat belonging to this nest.
