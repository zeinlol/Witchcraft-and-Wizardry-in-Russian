...
execute as @s[tag=!isApparating] at @s run function hp:spells/raycast/apparate_cast_p3
execute as @s[tag=!isApparating] as 0-0-10-0-3 at @s align xyz run tp @s ~.5 ~ ~.5 0 0

scoreboard players add 00000000-0000-0010-0000-000000000003 animTest 3

execute as 0-0-10-0-3 store result entity @s Pose.Head[1] float 1 run scoreboard players get @s animTest

# Appiration
execute as @s[tag=isApparating] run function hp:spells/spell/apparition/player_3/apparate
...
execute at 0-0-10-0-3 if block ~ ~-1 ~ #hp:apparate_ray run tag @s add spotNotValid
execute at 0-0-10-0-3 if block ~ ~-1 ~ barrier run tag @s add spotNotValid
execute at 0-0-10-0-3 unless block ~ ~ ~ #hp:apparate_ray run tag @s add spotNotValid
execute at 0-0-10-0-3 unless block ~ ~1 ~ #hp:apparate_ray run tag @s add spotNotValid

# If spot is invalid
execute as @s[tag=spotNotValid] run function hp:spells/spell/apparition/player_3/invalid_spot
...
execute as @s[tag=!isApparating,scores={appirationMarker=1..10}] run function hp:spells/spell/apparition/player_3/draw_indicator

# Init appiration
execute as @s[tag=hasPressedF,tag=!isApparating,tag=!spotNotValid] unless score @s apparating matches 1.. run function hp:spells/spell/apparition/player_3/init_apparate
