...
execute as @s[tag=use] as @e[tag=convOption] if score @s con = @p[tag=activePlayer] questConvOffset run data merge entity @s {CustomName:"{\"text\":\"±ˈˈ Мне нужны деньги на школьные принадлежности\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
