...
execute at @s as @e[limit=1,sort=nearest,tag=helperSpell,nbt={inGround:1b},distance=..0.45] run tag @s add hitSomething

# Detect if the trident stuck in an entity (the trident has dissapeared)
execute at @s[scores={idle=1..}] unless entity @e[sort=nearest,tag=helperSpell,distance=..0.45] run tag @s add hitEnemy
