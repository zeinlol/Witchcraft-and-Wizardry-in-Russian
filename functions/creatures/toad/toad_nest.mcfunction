...
execute at @s if entity @p[distance=..50] as @s[scores={timeSinceSpawn=0}] unless score @s spawnCount matches 0 unless score @s spawnCount matches 0 if score @s maxCreatureCount > @s creatureCount at @s run function hp:creatures/toad/nest_summon_toad
