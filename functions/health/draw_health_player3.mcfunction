...
execute unless score @s tmp = @s currentHealth run function hp:health/health_changed_player3

execute as @s[scores={playHealthAnim=1..}] run function hp:health/health_anim_player3
...
execute as @s[scores={poisoned=1}] run bossbar set minecraft:player3health color red
